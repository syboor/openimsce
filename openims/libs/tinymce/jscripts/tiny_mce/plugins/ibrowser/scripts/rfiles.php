<?php
        ...
        /* OSICT modifications:
	 * - path checking against server side stored base path
	 * - authentication (direct link will not work, only links from TinyMCE will work)
	 * - additional file extension checking
	 * 
	 * Every link to this script (form action, hyperlink, iframe src etc.) to this script must include the 
	 * "encodedsettings" parameter. The "clib" parameter must be exactly equal to the "imagepath" parameter
	 * of the encodedsettings object.
	 */
	...
	$clib = (isset($_REQUEST['clib']) ? $_REQUEST['clib'] : (isset($_REQUEST['ilibs']) ? $_REQUEST['ilibs'] : $cfg['ilibs'][0]['value']));	
        ...
        // OSICT
        if ($clib) {
          if ($clib != $decodedsettings['imagepath'] &&
              $clib != $decodedsettings['imagepath']."page_bestanden/" && // special case, subdirectory was created by ibrowser.php after settings were decoded.
              $clib != $decodedsettings['imagepath']."page_files/") {
            N_Die("Security violation");
          }
        } else {
          $clib = $decodedsettings['imagepath'];
        }
...
<?php
	...
	function renameImg($clib, $cfile, $nfile) {		
		...
		// osict: also check old file extension
	...
	// OSICT -------------------------------------------------------------------------
	// Check that $path is inside $dir (absolute dir). Use realpath to resolve ../ path
        // components before making the comparison. $path does not need to exist, but the
        // directory of $path should exist.
	function checkPath($dir, $path) {
		// The intent of this code is to stop certain malicious attacks that involve
		// adding "../" to url parameters.
		// If this function throws an error in the absence of any attacks, symlinks
		// may be to blame. Make certain that DOCUMENT_ROOT points to the "real" location
		// of OpenIMS instead of a symlink, and make certain no symlinks are used inside
		// the DOCUMENT_ROOT.
                $realpath = realpath(dirname($path)) . '/' . basename($path);
		if (basename($path) == ".." || substr($realpath, 0, strlen($dir)) != $dir) {
//                  echo "dir: $dir <br/>";
//                  echo "path: $path <br/>";
//                  echo "realpath: $realpath <br/>";

                  N_Die("Security violation (realpath)");
                }
        }
