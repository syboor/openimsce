...
class Dropbox_API {
    ...
    public function getFile($path = '', $root = null) {
        ...
        $result = $this->oauth->fetch('http://api-content.dropbox.com/0/files/' . $root . '/' . ltrim(str_replace ("%2F", "/", rawurlencode($path)),'/')); // NDV
        return $result['body'];
    ...
    public function putFile($path, $file, $root = null) {
        ...
        $url = 'http://api-content.dropbox.com/0/files/' . $root . '/' . trim($directory,'/'); // NDV
        $this->multipartFetch($url, $file, $filename);
        return true;
    }
    ...
    protected function multipartFetch($uri, $file, $filename) {
        ...
        $uri.='?file=' . rawurlencode ($filename); // NDV
