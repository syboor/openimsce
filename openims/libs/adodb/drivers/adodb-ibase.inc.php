		...
		if (is_array($sql)) {
			...
			if (is_array($iarr)) {
				if  (ADODB_PHPVER >= 0x4050) { // actually 4.0.4
					...
					$fnarr =& array_merge( array($sql) , (array)$iarr);
			...
			if (is_array($iarr)) {	
				if (ADODB_PHPVER >= 0x4050) { // actually 4.0.4
					...
					$fnarr =& array_merge( array($conn,$sql) , (array)$iarr);
					$ret = call_user_func_array($fn,$fnarr);
				} else {
					switch(sizeof($iarr)) {
...
class ADORecordset_ibase extends ADORecordSet
{
	...
	function _fetch() 
	{
		...
		if ($this->fetchMode == ADODB_FETCH_ASSOC) {
			...
			$this->fields =& array_merge((array)$this->fields,(array)$this->GetRowAssoc(ADODB_ASSOC_CASE));
		}
		return true;
	}
