...
// OSICT modifications:
// - pass on encodedsettings url param from TinyMCE to iBrowser popup
	...
	function iBrowser_open() {	
		...
		var regexp = new RegExp ("encodedsettings=([^&#]*)"); // OSICT
		var encodedsettings = regexp.exec(document.location.href)[1];	// OSICT
		if (ib.isMSIE) { // IE
			var rArgs = showModalDialog(ib.baseURL + '?encodedsettings=' + encodedsettings + '&img_path=' + image_dir + '&taal=' + taal, wArgs, 'dialogHeight:500px; dialogWidth:580px; scrollbars: no; menubar: no; toolbar: no; resizable: no; status: no;');  // OSICT
			...
			var wnd = window.open(ib.baseURL + '?encodedsettings=' + encodedsettings + '&img_path=' + image_dir + '&taal=' + taal + '&editor=' + ib.editor + '&callback=iBrowser_callback', 'ibrowser', 'status=no, modal=yes, width=625, height=530'); // OSICT
...
// 20110920 KvD ignore when hspace or vspace are not defined (FF 6 or Chrome 1.4)
    if (oImageElement.hspace >= 0 && typeof(oImageElement.attributes['hspace']) != "undefined") { // (-1 when not set under gecko for some reason)
//    if (oImageElement.hspace >= 0) { // (-1 when not set under gecko for some reason)
    ...
    if (oImageElement.vspace >= 0 && typeof(oImageElement.attributes['vspace']) != "undefined") { // // (-1 when not set under gecko for some reason)
